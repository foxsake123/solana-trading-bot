# simple_dashboard.py - Robust, simplified dashboard for Solana Trading Bot
...
def get_trades_by_type(conn, is_simulation=True):
    ...
    try:
        ...
        st.error(f"Error getting trades: {e}")
    ...
    for control_file in control_files:
        if os.path.exists(control_file):
            try:
                ...
                st.error(f"Error loading {control_file}: {e}")
...
def main():
    ...
    # Real Trading Tab
    with tabs[1]:
        st.markdown("<div class='real-header'>REAL TRADING DASHBOARD</div>", unsafe_allow_html=True)
        
        # Warning if simulation mode is enabled
        if bot_settings.get('simulation_mode', True):
            st.warning("⚠️ The bot is currently in simulation mode. No real trades will be executed.")
    ...
    # Simulation Tab
    with tabs[2]:
        st.markdown("<div class='sim-header'>SIMULATION DASHBOARD</div>", unsafe_allow_html=True)
    ...
    with tabs[3]:
        ...
        if st.button("Save Trading Parameters"):
            # Update settings - store as decimals not percentages
            bot_settings['take_profit_target'] = take_profit / 100  # Convert to decimal 
            bot_settings['stop_loss_percentage'] = stop_loss / 100  # Convert to decimal
