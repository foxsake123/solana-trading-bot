...
try:
    ...
    # Create minimal BotConfiguration if import fails
...
# Try to import MLModel
try:
    from ml_model import MLModel
except ImportError:
    # Create dummy MLModel if import fails
    class MLModel:
        def __init__(self):
...
# Try to import SolanaTrader - handle different possible locations
try:
    from solana_trader import SolanaTrader
except ImportError:
    try:
        # Try from trading module
        from trading.solana_trader import SolanaTrader
    except ImportError:
        try:
            # Try from trading_bot
            from trading_bot import SolanaTrader
        except ImportError:
            # Create dummy SolanaTrader as fallback
            class SolanaTrader:
                def __init__(self):
...
try:
    ...
    class Database:
        ...
        def get_tokens(self, limit=30):
            return pd.DataFrame()
            
        def reset_database(self):
            return True

# Try to import TokenAnalyzer and BirdeyeAPI
try:
...
async def calculate_simulation_stats(trades_df, sol_price_usd):
    ...
    try:
        ...
        # If we have gain_loss_sol column, use that directly
        if 'gain_loss_sol' in sim_trades.columns:
            # Use gain/loss metrics directly from the database
            sell_trades = sim_trades[sim_trades['action'] == 'SELL']
            if not sell_trades.empty:
                total_pl_sol = sell_trades['gain_loss_sol'].sum()
                
                # Count winning and losing trades
                winning_trades = (sell_trades['gain_loss_sol'] > 0).sum()
                losing_trades = (sell_trades['gain_loss_sol'] < 0).sum()
                
                # Get best and worst multiples
                if 'price_multiple' in sell_trades.columns:
                    best_multiple = sell_trades['price_multiple'].max()
                    worst_multiple_val = sell_trades['price_multiple'].min()
                    if worst_multiple_val > 0:
                        worst_multiple = worst_multiple_val
                
                # Get total buy amount for ROI calculation
                buys = sim_trades[sim_trades['action'] == 'BUY']
                total_investment = buys['amount'].sum()
                
                # Calculate ROI
                roi_percentage = (total_pl_sol / total_investment * 100) if total_investment > 0 else 0.0
                
                # Update stats
                stats['total_pl_sol'] = total_pl_sol
                stats['total_pl_usd'] = total_pl_sol * sol_price_usd
        ...
        # Older method - calculate from buy/sell transactions
...
def calculate_stats(trades_df, positions_df):
    ...
    try:
        ...
        if total_completed > 0:
            ...
            # If gain_loss_sol column exists, use it
...
def get_bot_control():
    ...
    try:
        ...
        return {
            ...
            'min_investment_per_token': 0.5,  # Default minimum investment
            'max_investment_per_token': 1.0,  # Default maximum investment
...
def update_bot_control(control):
    ...
    try:
        import json
        os.makedirs(os.path.dirname(BotConfiguration.BOT_CONTROL_FILE), exist_ok=True)
...
# Rest of your dashboard.py file contents...
# Include the display_dashboard function and everything after it
...
def display_dashboard():
    ...
    with st.sidebar:
        ...
        with tabs[0]:  # Core parameters
            ...
            # Add minimum investment parameter
            min_inv = st.number_input(
                "Min Investment per Token (SOL)",
                min_value=0.01,
                max_value=float(control.get('max_investment_per_token', 1.0)) - 0.01,
                value=float(control.get('min_investment_per_token', 0.5)),
                step=0.05,
                help="Minimum SOL to invest in a single token"
            )
            
            max_inv = st.number_input(
                "Max Investment per Token (SOL)",
                min_value=min_inv + 0.01,
                max_value=5.0,
        ...
        if st.button("Update Trading Parameters", type="primary"):
            ...
            control['min_investment_per_token'] = min_inv
    ...
    with tab3:
        ...
        if data['trades_df'].empty:
            ...
            # Add trade performance display for SELL trades with gain/loss metrics
            if not trades_display.empty:
                # Check if trades have gain/loss columns
                if ('gain_loss_sol' in trades_display.columns and 
                    'percentage_change' in trades_display.columns and 
                    'price_multiple' in trades_display.columns):
                    
                    # Create a filtered view of SELL trades to show gain/loss metrics
                    sell_trades = trades_display[trades_display['action'] == 'SELL'].copy()
                    
                    if not sell_trades.empty:
                        st.subheader("Trade Performance (SELL Transactions)")
                        
                        # Format the columns for better display
                        sell_trades['gain_loss_sol'] = sell_trades['gain_loss_sol'].round(6)
                        sell_trades['percentage_change'] = sell_trades['percentage_change'].round(2)
                        sell_trades['price_multiple'] = sell_trades['price_multiple'].round(2)
                        
                        # Display the table
                        st.dataframe(sell_trades)
                        
                        # Add a summary of trade performance
                        st.subheader("Trade Performance Summary")
                        
                        summary_col1, summary_col2, summary_col3 = st.columns(3)
                        
                        with summary_col1:
                            total_gain_loss = sell_trades['gain_loss_sol'].sum()
                            avg_gain_loss = sell_trades['gain_loss_sol'].mean()
                            
                            st.metric(
                                "Total Gain/Loss (SOL)", 
                                f"{total_gain_loss:.6f}",
                                delta=f"{total_gain_loss:.6f}",
                                delta_color="normal"
                            )
                            
                            st.metric(
                                "Average Gain/Loss per Trade (SOL)",
                                f"{avg_gain_loss:.6f}"
                            )
                        
                        with summary_col2:
                            avg_pct_change = sell_trades['percentage_change'].mean()
                            win_rate = (sell_trades['gain_loss_sol'] > 0).mean() * 100
                        ...
                        with summary_col3:
                            avg_multiple = sell_trades['price_multiple'].mean()
                            best_multiple = sell_trades['price_multiple'].max()
                            ...
                            st.metric(
                                "Best Price Multiple",
                                f"{best_multiple:.2f}x"
                            )
                        
                        # Add a histogram of gain/loss distribution
                        if len(sell_trades) >= 3:  # Only show if we have enough data
                            ...
                            fig = px.histogram(
                                ...
                                labels={'percentage_change': 'Return (%)'}
                            )
