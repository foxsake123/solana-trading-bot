"""
Enhanced Trading Bot Dashboard with ML Analytics - v11 FIXED
Fixes: 
...
4. FIXED parameter ranges to accommodate actual config values
5. Single source of truth from bot_control.json
"""
...
def main():
    ...
    with tabs[3]:
        ...
        # Trading parameters with FIXED RANGES
        st.markdown("#### Core Trading Parameters")
        
        tp_col1, tp_col2, tp_col3 = st.columns(3)
        
        with tp_col1:
            # FIXED: Increased max_value to accommodate 25x target
            take_profit = st.number_input(
                "Take Profit Target",
                min_value=1.1,
                max_value=100.0,  # FIXED: Increased from 10.0 to 100.0
                value=float(bot_settings.get('take_profit_target', 2.5)),
                step=0.1,
                help="Exit position when price reaches this multiple (e.g., 25 = 25x return)"
            )
        ...
        with tp_col2:
            ...
            stop_loss = st.number_input(
                "Stop Loss (%)",
                min_value=1.0,  # FIXED: Changed from 5.0 to 1.0
                max_value=50.0,
        ...
        with tp_col3:
            # FIXED: Better precision for small values
            max_investment = st.number_input(
                "Max Investment (SOL)",
                min_value=0.001,  # FIXED: Reduced from 0.01 to 0.001
                max_value=10.0,
                value=float(bot_settings.get('max_investment_per_token', 0.1)),
                step=0.001,  # FIXED: Reduced step for precision
                format="%.3f",
                help="Maximum SOL to invest per token"
            )
        
        # Add slippage tolerance and other parameters
        ...
        with add_col2:
            slippage_display = bot_settings.get('slippage_tolerance_display', 30.0)
            slippage_tolerance = st.number_input(
                "Slippage Tolerance (%)",
                min_value=0.1,
                max_value=50.0,
                value=float(slippage_display),
                step=0.1,
                help="Maximum acceptable slippage (stored as decimal in config)"
            )
        ...
        with add_col3:
            min_volume = st.number_input(
                ...
                min_value=0.0,
                max_value=1000000.0,
                value=float(bot_settings.get('MIN_VOLUME', 10000.0)),
                step=1000.0
            )
        
        # Save parameters with proper conversion
        if st.button("💾 Save All Parameters", type="primary"):
            ...
            updated_settings.update({
                ...
                'stop_loss_percentage_display': stop_loss,  # Store display value temporarily
                'min_investment_per_token': min_investment,
                'max_investment_per_token': max_investment,
                'slippage_tolerance_display': slippage_tolerance,  # Store display value temporarily
                'MIN_VOLUME': min_volume
            })
            
            # Save to bot_control.json (using the file that was loaded from)
            control_file = bot_settings.get('_loaded_from', 'bot_control.json')
            if save_bot_settings(updated_settings, control_file):
                st.success(f"✅ Parameters saved successfully to {control_file}!")
                st.info("🔄 Settings will be applied on next bot restart")
                ...
                with st.expander("📋 Saved Configuration"):
                    st.json({
                        'stop_loss_percentage': stop_loss / 100,  # Show decimal value
                        'slippage_tolerance': slippage_tolerance / 100,  # Show decimal value
                        'take_profit_target': take_profit,
                        'max_investment_per_token': max_investment,
                        'min_investment_per_token': min_investment,
                        'MIN_VOLUME': min_volume
