...
class JupiterClient:
    ...
    async def get_quote(self, input_mint, output_mint, amount, slippage=0.5):
        ...
        try:
            ...
            params = {
                ...
                "amount": str(amount),  # Make sure amount is a string
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(self.quote_endpoint, params=params, timeout=30) as response:
                    if response.status != 200:
                        logger.error(f"Jupiter API error: {response.status}, {await response.text()}")
                        return None
    ...
    async def prepare_swap_transaction(self, quote, user_public_key):
        ...
        try:
            ...
            payload = {
                ...
                "wrapUnwrapSOL": True  # Handle SOL wrapping/unwrapping
            }
            
            logger.info(f"Preparing Jupiter swap for user: {user_public_key}")
            
            # Send request
            async with aiohttp.ClientSession() as session:
                async with session.post(self.swap_endpoint, json=payload, timeout=30) as response:
                    if response.status != 200:
                        logger.error(f"Jupiter API error: {response.status}, {await response.text()}")
                        return None
                    ...
                    if data and 'swapTransaction' in data:
                        logger.info("Successfully prepared swap transaction")
    ...
    async def execute_swap(self, swap_transaction, keypair, rpc_endpoint):
        ...
        try:
            ...
            # Extract transaction message from the swap response
            tx_data = swap_transaction.get('swapTransaction')
            if not tx_data:
                logger.error("No transaction data in swap response")
                return None
            
            logger.info("Deserializing transaction data...")
            ...
            try:
                # Try first as VersionedTransaction (newer format)
                tx = VersionedTransaction.from_bytes(tx_bytes)
                logger.info("Created VersionedTransaction")
            except Exception:
                # Fall back to legacy Transaction
                tx = Transaction.from_bytes(tx_bytes)
                logger.info("Created legacy Transaction")
            ...
            logger.info(f"Created RPC client for {rpc_endpoint}")
            ...
            if isinstance(tx, VersionedTransaction):
                ...
                # Send transaction
                client_signature = await client.send_transaction(tx, keypair)
            else:
                ...
                # Send transaction
                client_signature = await client.send_transaction(tx)
            
            logger.info(f"Transaction sent: {client_signature}")
            
            # Confirm transaction
            logger.info("Waiting for confirmation...")
            await client.confirm_transaction(client_signature)
            logger.info("Transaction confirmed!")
            
            return str(client_signature)
