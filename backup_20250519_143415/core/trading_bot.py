...
class TradingBot:
    def __init__(self, config, db=None, token_scanner=None, token_analyzer=None, trader=None):
        ...
        import traceback
        print(f"TradingBot init with trader.simulation_mode = {trader.simulation_mode if trader else 'None'}")
    ...
    async def evaluate_token(self, token: Dict[str, Any]) -> bool:
        ...
        min_price_change_1h = self.control.get('MIN_PRICE_CHANGE_1H', 1.0)
    ...
    async def trade_token(self, token: Dict[str, Any]):
        ...
        max_investment = self.control.get('max_investment_per_token', 1.0)
