# reset_dashboard.py - Reset to original dashboard without ET conversion
...
def reset_dashboard():
    """
    Create a clean version of the dashboard without ET conversion
    """
    # Backup the original file first
    if os.path.exists('enhanced_dashboard.py'):
        backup_name = 'enhanced_dashboard_backup.py'
        try:
            shutil.copy('enhanced_dashboard.py', backup_name)
            print(f"✅ Backed up original dashboard to {backup_name}")
        except Exception as e:
            print(f"❌ Error backing up file: {e}")
    
    # Create a new simplified dashboard file
    code = """# Simplified dashboard without timestamp conversion issues
import asyncio
import pandas as pd
...
async def get_sol_price(trader):
    ...
    try:
        # Try to use alternative price source first
...
async def update_token_prices(positions_df, db, trader):
    ...
    \"\"\"
    # Import logger at function level
    from logging import getLogger
    local_logger = getLogger('trading_bot.dashboard')
    ...
    local_logger.info(f"Current SOL price for calculations: ${sol_price_usd}")
    ...
    local_logger.info(f"Updated {len(updated_df)} positions with proper USD values")
        ...
        if total_completed > 0:
            ...
            for contract in sell_trades['contract_address'].unique():
                ...
                if not buys.empty and not sells.empty:
                    ...
                    buy_time_str = buys.iloc[0]['timestamp']
                    sell_time_str = sells.iloc[-1]['timestamp']
...
def display_dashboard():
    ...
    # Display the rest of the tabs
    # ... (remaining code for other tabs)
...
"""
    ...
    # Create a new simplified dashboard file
    with open('simplified_dashboard.py', 'w', encoding='utf-8') as f:
        f.write(code)
    
    print("✅ Created simplified_dashboard.py without ET conversion")
    print("To run the simplified dashboard, use:")
    print("  streamlit run simplified_dashboard.py")
