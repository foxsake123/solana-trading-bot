# fix_simplified_token_scanner.py
import os

def fix_token_scanner():
    """Update simplified_token_scanner to use the BirdeyeAPI (DexScreener)"""
    
    content = '''# simplified_token_scanner.py
import asyncio
import logging
...
'''
    ...
    for file_path in ['simplified_token_scanner.py', 'core/simplified_token_scanner.py']:
        if os.path.exists(os.path.dirname(file_path)) or os.path.dirname(file_path) == '':
