...
def main():
    ...
    with tabs[0]:
        ...
        if not all_trades.empty:
            ...
            all_trades['type'] = all_trades.apply(
                lambda row: "Simulation" if row.get('is_simulation', False) or row['contract_address'].startswith('Sim') else "Real",
                axis=1
            )
    ...
    with tabs[1]:
        ...
        if not positions_real.empty:
            ...
            if 'entry_time' in display_positions and not display_positions['entry_time'].isnull().all():
            ...
            display_cols = ['ticker', 'amount', 'avg_buy_price', 'current_price', 'unrealized_pl', 'pl_percent', 'entry_time']
    ...
    with tabs[3]:
        ...
        settings_tabs = st.tabs(["General", "Trading Parameters", "Token Filters", "API Keys"])
...
if __name__ == "__main__":
    ...
    with tabs[2]:
        ...
        if not positions_sim.empty:
            ...
            with control_col1:
                if st.button("Generate New Simulation Tokens"):
                    st.success("New simulation tokens would be generated here")
            
            with control_col2:
                if st.button("Fast-Forward Simulation"):
                    st.success("Simulation would be fast-forwarded here")
        else:
            ...
            st.markdown("""
            No active simulation positions. You can:
            
            1. Generate new simulation tokens to practice trading
            2. Adjust simulation parameters in the Settings tab
            3. Enable simulation mode if it's currently disabled
            """)
            
            if st.button("Generate Simulation Tokens"):
                st.success("New simulation tokens would be generated here")
